  ...
  describe('isConfigured()', () => {
    it('returns true when serverKey exists in mcpServers', async () => {
      (fs.readFileSync as jest.Mock).mockReturnValue(
        JSON.stringify({ mcpServers: { 'vscode-fezzan': { url: 'http://127.0.0.1:3333/mcp' } } }),
      );
  ...
  describe('configure()', () => {
    ...
    it('creates new config with correct structure when file does not exist', async () => {
      ...
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        ...
        JSON.stringify({ mcpServers: { 'vscode-fezzan': { type: 'http', url: 'http://127.0.0.1:3333/mcp' } } }, null, 2),
      );
    });
