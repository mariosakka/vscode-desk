# Relay MCP — Agent Quick Start

Relay is a VS Code extension that manages tabbed bookmarks and `.relay` pages.
It exposes a local JSON-RPC 2.0 server so you can read and write its data programmatically.

## Connect

The server runs on `http://localhost:3333/mcp` while VS Code is open with Relay installed.
...
Pages are separate — they live as `.relay` files in `relay-pages/` inside the open workspace.

## The typical loop

```
list_tabs           → find or create the right tab
...
{ "jsonrpc": "2.0", "method": "tools/call",
  "params": { "name": "list_tabs", "arguments": {} }, "id": 1 }

// → [{ "id": "tab_abc123", "name": "Work", "bookmarkCount": 3 }]
...
The user can then open `auth-flow.relay` from **Relay: Open Page** in the VS Code Command Palette,
or by adding a bookmark with `url: "relay-page:auth-flow.relay"`.

## Key rules

- **IDs are opaque** — always call `list_tabs` / `list_bookmarks` to get current IDs; never guess or cache them across sessions.
